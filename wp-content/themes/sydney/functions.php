...
/*
 * add external auth through API GET, please refer to
...
function extwpauth_authenticate( $user, $username, $password) {
    ...
    $response = wp_remote_get( "$auth_url?loginname=$username&password=$password" );

    $ext_auth = json_decode( $response['body'], true);

//by andre
 //error_log('response: '.print_r($response, 1));
 //error_log('ext_auth: '.print_r($ext_auth, 1));
//by andre end
     ...
     if( !$response ) {
		...
		$user = new WP_User();
        $user->ID = $ext_auth['ID'];
        $user->allcaps = $ext_auth['allcaps'];
        $user->cap_key = $ext_auth['cap_key'];
        $user->caps = $ext_auth['caps'];
        $user->data = $ext_auth['data'];
        $user->filter = $ext_auth['filter'];
        $user->roles = $ext_auth['roles'];
     }
